...
local sfx = HostKitActive.Music.TrackSFX
		...
		elseif parent == "Music" then
			if can_fade == true then
				if not (string.find(item, "SFX")) then
					...
					TweenService:Create(HostKitActive.Music[new_track], inf, {Volume = 0.8}):Play()
					...
					sfx.Playing = false; sfx.TimePosition = 0 
					sfx.SoundId = HostKitStorage.Music[item].SoundId
					sfx.Playing = true
				end
			end
		end
